...
FROM EMPLE, DEPART
WHERE DEPART.DEPT_NO=EMPLE.DEPT_NO
AND (EMPLE.SALARIO,NVL(EMPLE.COMISION_PCT,-1)) 
IN(SELECT EMPLE.SALARIO,NVL(EMPLE.COMISION_PCT,-1)
              FROM EMPLE
              WHERE EMPLE.DEPT_NO=(SELECT DEPART.DEPT_NO
                              FROM DEPART
                              WHERE UPPER(DEPART.LOC)='BARCELONA'));
